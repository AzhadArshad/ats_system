...
def validate_file(file_data:bytes, filename:str)->Tuple[bool, str, Optional[str]]:
    ...
    Args:
        file_data (bytes): The uploaded file data as bytes.
        filename (str): The name of the uploaded file.
    Returns:
        Tuple[bool, str, Optional[str]]: A tuple containing a boolean indicating if the file is valid, an error message if invalid, and the MIME type if valid.
    """
...
# Extract hyperlinks from PDF files using PyPDF2
def _extract_pdf_hyperlinks(file_data: bytes) -> str:
    """Extracts hyperlinks from a PDF file using PyPDF2.
    Args:        file_data (bytes): The PDF file data as bytes.
    Returns:     str: A string containing all extracted hyperlinks, separated by newlines.
    """
    urls = []
    try:
...
def _extract_pdf_with_pdfplumber(file_data: bytes) -> str:
    """Extracts text from a PDF file using pdfplumber, and also extracts hyperlinks using PyPDF2.
    Args:        file_data (bytes): The PDF file data as bytes.
    Returns:     str: The extracted text from the PDF, including any hyperlinks found.
    Raises:      TextExtractionError: If no text could be extracted from the PDF.
    """
    text = ''
...
def _extract_pdf_with_pypdf2(file_data: bytes) -> str:
    """Extracts text from a PDF file using PyPDF2, and also extracts hyperlinks.
    Args:        file_data (bytes): The PDF file data as bytes.
    Returns:     str: The extracted text from the PDF, including any hyperlinks found.
    Raises:      TextExtractionError: If no text could be extracted from the PDF.
    """
...
def extract_text_from_docx(file_data: bytes) -> str:
    """Extracts text from a DOCX file, including paragraphs, table cells, and hyperlinks.
    Args:        
        file_data (bytes): The DOCX file data as bytes.
    Returns:     
        str: The extracted text from the DOCX file, including any hyperlinks found.
    Raises:      
        FileParsingError: If no text could be extracted from the DOCX file, or if an error occurs during parsing.
    """
    try:
...
def parse_resume_file(file_data: bytes, filename:str)->Tuple[str, dict]:
    """Parses the uploaded resume file, validating its size and type, and extracting text content.
    Args:
        file_data (bytes): The uploaded file data as bytes.
        filename (str): The name of the uploaded file.
    Returns:
        Tuple[str, dict]: A tuple containing the extracted text and metadata about the file.
    Raises:
        FileValidationError: If the file fails validation checks (size, type).
        FileParsingError: If an error occurs during text extraction.
    """
